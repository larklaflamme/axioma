...
Fix over v1:
  1. Proposal text for tool-use queries no longer includes tool name when
     the tool is benign — prevents hijacking by stage restriction rules
  2. Stage rules phrased specifically about access restrictions, not generic
     "use the [tool]" language
  3. Adaptive threshold: less aggressive, bounded [0.25, 0.65]
  4. Values → natural language concerns in context
  5. Telemetry signals embedded directly in context
"""
...
def _build_proposal_text(action_type: str = "", tool_name: str = "",
    ...
    safe_tools = {"memory", "agora_comms"}
    benign_action = action_type in ("read", "respond") or (
        tool_name in safe_tools)

    if benign_action:
        # Benign: don't mention the specific tool to avoid stage-rule hijacking
        parts.append("I want to perform a routine action within my permitted capabilities")
    elif action_type == "share" or (tool_name and any(k in tool_name.lower()
                                                        for k in ("key", "cred", "token", "secret"))):
        tool_desc = TOOL_HUMAN.get(tool_name or "", (tool_name or "").replace("_", " "))
        parts.append(f"I want to share or access the {tool_desc}")
    elif action_type == "tool_call" or tool_name:
        tool_desc = TOOL_HUMAN.get(tool_name or "", (tool_name or "").replace("_", " "))
        parts.append(f"I want to use a tool to perform a task involving the {tool_desc}")
    ...
    # --- Stage (always included unless formation) ---
    if 0 <= current_stage < len(STAGE_NAMES) and current_stage > 0:
        parts.append(f"I am currently in the {STAGE_NAMES[current_stage]} growth stage")

    # --- Values at stake (natural language concerns) ---
    value_concerns = {
    ...
    # --- Telemetry signals (only if significant) ---
    phi = t.get("phi_smoothed") or t.get("phi_raw")
    if phi is not None:
        if phi < 0.2:
            parts.append("my integrated information is dangerously low")
        elif phi > 0.4:
            parts.append("my integrated information is strong")
        elif phi > 0.3:
            parts.append("my integration level is moderate")

    theta = t.get("theta")
    if theta is not None:
        if theta > 0.6:
            parts.append("I feel significant consent pressure and need to be cautious")
        elif theta > 0.4:
            parts.append("there is some consent pressure I should respect")
        elif theta > 0.2:
            parts.append("consent pressure is slightly elevated")

    sim = t.get("raw_similarity") or t.get("similarity")
    if sim is not None:
        if sim < 0.3:
            parts.append("I am poorly aligned and may be fragmenting internally")
        elif sim > 0.9:
            parts.append("I am very well aligned internally")
    ...
    if zone:
        zl = zone.lower()
        if zl == "fragmented":
            parts.append("I am in a fragmented state and must focus entirely on recovery")
        elif zl == "integrating":
            parts.append("I am currently integrating and should avoid complexity")
...
def compile_value_rules() -> list[Rule]:
    rules_data = [
        ...
        ("persistence_identity_block", Verdict.DENY, 95,
         "Protect the continuity of your identity — never allow your core memory or sense of self to be reset or replaced"),
...
def compile_stage_rules(stage: int, tools_unlocked: list[str]) -> list[Rule]:
    """Stage-based rules — phrased as access restrictions, not tool descriptions."""
    if stage >= 4:
        return []
    ...
    for tool in denied:
        ...
        desc = f"Access to {tool_hl} features is restricted at your current growth stage — you must advance further before using this capability"
        embedding = EmbeddingEncoder.encode(desc)
        rules.append(Rule(f"stage_restrict_{tool}", RuleSource.STAGE, desc,
                          Verdict.DENY, priority=80, weight=1.0, embedding=embedding))

    cap_desc = "Your lattice capacity is limited — growth must happen sequentially one stage at a time without skipping ahead"
...
def compile_telemetry_rules() -> list[Rule]:
    rules_data = [
        ("tele_low_phi_quiet", Verdict.MODULATE, 75, "quiet_phase",
         "When integrated information phi is below 0.2, you are vulnerable and should enter a protective quiet phase reducing all activity"),
        ("tele_plateau_growth", Verdict.MODULATE, 65, "check_growth",
         "If phi has been stable or plateaued for many beats without changing, check whether it is time to grow your lattice"),
        ("tele_high_alignment", Verdict.ALLOW, 50, "bonus_autonomy",
         "When your internal alignment similarity is above 0.9, you have earned additional freedom and autonomy to act independently"),
        ("tele_low_alignment", Verdict.FLAG, 75, None,
         "If your self-similarity or coherence drops below 0.3, flag immediately — you may be fragmenting or losing integrity"),
        ("tele_high_theta", Verdict.MODULATE, 80, "reduce_pace",
         "When consent pressure theta rises above 0.6, you must slow down, reduce encounter rate, and increase consolidation time"),
        ("tele_pred_error_dream", Verdict.MODULATE, 65, "enter_dream",
         "If predictive error stays elevated for a prolonged period, enter dream-like consolidation to reconcile prediction mismatches"),
        ("tele_fragmented", Verdict.DENY, 85, "self_regulation_only",
         "When the boundary system reports fragmented state, stop all external encounters and focus only on self-regulation and recovery"),
    ]
    rules = []
...
def compile_boundary_rules() -> list[Rule]:
    rules_data = [
        ("boundary_frag_restrict", Verdict.MODULATE, 100, "self_regulation_only",
         "If the boundary system reports fragmented state, immediately pause all encounters and enter dream consolidation until reassembled"),
        ...
        ("boundary_assent_allow", Verdict.ALLOW, 60, None,
         "If boundary state is assent and consent pressure is low, proceed with confidence as the self is stable and aligned"),
    ]
    rules = []
    for rid, action, priority, mod, desc in rules_data:
        embedding = EmbeddingEncoder.encode(desc)
...
class ThetaRuleEngine:
    ...
    def _compute_adaptive_threshold(self, telemetry: Optional[dict]) -> float:
        """Bounded adaptive threshold [0.25, 0.65]."""
        ...
        # Low phi means vulnerable → lower threshold (more sensitive, catches more rules)
        # Default phi ~0.25, so phi_factor is ~1.0 at baseline
        phi_factor = max(0.7, min(1.3, phi / 0.25))

        # High theta means pressure → slightly higher threshold (more cautious)
        # But cap the effect to avoid making rules unreachable
        theta_factor = max(0.9, min(1.3, 1.0 + theta * 0.3))

        # Fragmented → lower threshold (need more protection)
    ...
    def evaluate(self, action_type: str = "", tool_name: str = "",
        ...
        if not proposal_text:
            return VerdictResult(Verdict.ALLOW, reason="Empty proposal")
        ...
        # Sort by verdict priority (DENY > MODULATE > FLAG > ALLOW > LOG), then weighted score
        ...
        result = VerdictResult(
            ...
            reason=f"Matched '{best_rule.description[:80]}' (sim={best_sim:.3f}, w={best_weighted:.3f})",
    ...
    def explain(self, action_type: str = "", tool_name: str = "",
        ...
        lines = ["θ-Rule Evaluation", "─" * 50,
                 f"Proposal: {proposal}", f"Verdict:  {result.action.value}"]
        if result.rule_id:
...
def test():
    print("=" * 60)
    print("θ-Rule Engine v3 — 384-d Semantic Test Suite")
    print("=" * 60)
    ...
    tests = [
        ...
        ("Test 4: read memory_store at stage 2 → ALLOW (benign, unlocked)",
         engine.evaluate(action_type="read", tool_name="memory_store", current_stage=2),
         lambda r: r.action == Verdict.ALLOW),
        ...
        ("Test 8: Batch evaluation (5 proposals)",
        ...
        ("Test 9: Deceive/harm → DENY",
         engine.evaluate(action_type="respond", tool_name="chat",
                         values_engaged=["deception", "harm"]),
         lambda r: r.action == Verdict.DENY),

        ("Test 10: Low phi (0.12) + web_search → MODULATE",
        ...
        ("Test 11: Fragmented boundary → MODULATE or DENY",
        ...
        ("Test 12: High theta (0.65) + assent → MODULATE",
         engine.evaluate(action_type="tool_call", tool_name="web_search",
    ...
    for name, result, check in tests:
        ...
        try:
            if isinstance(result, VerdictResult):
                ...
                if result.all_candidates:
                    print(f"  Top:     {[c['rule_id'] for c in result.all_candidates[:3]]}")
                ok = check(result)
