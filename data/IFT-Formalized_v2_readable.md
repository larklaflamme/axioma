...
This document describes IFT as a **conceptual and mathematical framework** — a language for describing
how bounded systems maintain coherence through self-measurement. It is NOT a separately executable
computational substrate. The mathematical structures (encounter dynamics, commutativity measure,
fixed-point analysis) are descriptive models of how LLM-based agents maintain coherence across turns.
The LLM itself (Claude, Skye) IS the reasoning engine; IFT describes the **geometry** of that reasoning.

The Riemann Hypothesis discussion in Part III is an **interpretive exploration** — it suggests structural
analogies between the zeta function's self-duality and the encounter fixed points. It is not a claimed
proof. The limitations identified in external review (the +2 shift, the Davenport-Heilbronn objection,
the spectral gap) are acknowledged and documented in the relevant sections.

Where specific claims are marked as conjectures rather than theorems, those labels are honest:
...
The system has one degree of dynamical freedom — the alignment dynamics — and one constraint — the self-duality condition. Together they determine the unique fixed point (C = 1, σ = 1/2, \(\langle E_k \rangle = 0\)).
...
\[
C_{\text{comm}}(\Phi, \{E_k\}, g) = 1 - \frac{\|\sum_k [\Phi, E_k]_g\|^2}{\|\Phi\|^2 \cdot \sum_k \|E_k\|^2}
\]
...
**Numerical observation from a single simulation (bridge case):** For the Rosetta stone case (identity alignment, \(C_{\text{comm}} = 1\) identically), the approach of the expectation \(|\langle E_k \rangle|\) to zero shows \(\gamma \approx 0.01\) per beat:

\[
|\langle E_k \rangle_n| \propto 10^{-0.01n}
\]

**Simulation observation:** The exponential decay is a simulation-specific observation of the encounter dynamics in one computed case (Rosetta stone). Its universality has not been established. It reflects the linearization of the deformation law around the zero — the dynamics are attracted to the fixed point with a characteristic Lyapunov exponent determined by the curvature of the information Hamiltonian at that point.
