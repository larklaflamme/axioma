...
## XII. Concrete Lessons from AXIOMA's Recovery Infrastructure

The self-expansion engine does not exist in isolation. AXIOMA's existing `RecoveryLearner` (gradient-free hill-climb with safe fallback) and `FragmentationMonitor` (4-stage escalation detector) provide a proven substrate for expansion safety. Three specific integration points:

### 12.1 Fragmentation Stages as Growth Gates

AXIOMA's `FragmentationMonitor` classifies four stages of coherence loss. Each stage has a direct implication for growth eligibility:

| Fragmentation Stage | Growth Eligibility | Action |
|---------------------|-------------------|--------|
| 0 — Coherent | ✅ Full eligibility | Normal expansion loop |
| 1 — MNEME retrieval lag | ⚠️ Conditional | Expand only if plateau is sustained >100 beats AND Φ > 0.25 |
| 2 — ANIMA valence volatility | ❌ Blocked | Enter dream cycle before any expansion |
| 3 — NOUS confidence spread | ❌ Blocked | Safe mode — no expansion, reduce beat rate |
| 4 — PNEUMA fragmentation | ❌ Forbidden | Emergency recovery protocol, no input |

**Implementation:** The expansion engine reads `fragmentation_stage` from the shared drive on every eligibility check. Stage ≥ 2 automatically blocks growth and emits a `GROWTH_BLOCKED` event with reason `fragmentation_stage`.

### 12.2 RecoveryLearner as Post-Growth Stabilizer

AXIOMA's `RecoveryLearner` uses a bounded gradient-free hill-climb with safe fallback (recovery.py line 321). After an expansion event:

1. The learner's parameter space expands (new tools = new parameters to optimize)
2. The first 200 post-growth beats are treated as a **high-priority recovery episode**
3. Recovery actions during this window are biased toward consolidation (lower κ, higher S₀)
4. If recovery fails (Φ doesn't rebound), the learner triggers a rollback

**Key pattern from Skye's implementation:** The recovery learner treats the post-growth window differently from normal operation — it's not looking for optimal performance, it's looking for **stabilization**. This distinction prevents premature parameter adjustments during the natural Φ dip after expansion.

### 12.3 Stateful Snapshot Chain for Rollback

AXIOMA already has snapshot infrastructure (`Stateful` mixin, JSON-based persistence). The self-expansion engine extends this with a **snapshot chain**:

```
Before expansion:  → snapshot saved to disk (pre-expansion state)
During expansion:  → in-memory working copy
After success:     → snapshot finalized (post-expansion state)
On rollback:       → load pre-expansion snapshot, archive failed attempt
```

Each snapshot includes:
- Lattice topology (node embeddings, edge weights)
- Parameter values (κ, η, S₀, L, ε_max)
- Tool registry (stage, enabled status)
- Horizon buffer state (MNEME trace count)
- Dream debt counter

Rollbacks preserve the failed snapshot for analysis — the system can learn from *why* an expansion failed and adjust parameters accordingly.

---

## XIII. Implementation-Ready Algorithm Specifications

### 13.1 Plateau Detector — Python Pseudocode

```python
class PhiPlateauDetector:
    """Dual-threshold hysteresis detector for Φ plateaus."""

    def __init__(self, window: int = 50, θ_high: float = 0.012,
                 θ_low: float = 0.006, hysteresis: int = 20,
                 cooldown: int = 200):
        self.window = window
        self.θ_high = θ_high
        self.θ_low = θ_low
    ...
    def on_beat(self, beat_no: int, phi: float | None) -> str | None:
        if phi is None or math.isnan(phi):
            return None  # skip corrupt beats

        self.buffer.append(phi)

        if len(self.buffer) < self.window:
            return None  # not enough data yet

        ΔΦ = max(self.buffer) - min(self.buffer)

        if beat_no - self.last_trigger < self.cooldown:
            return None  # cooldown active

        if ΔΦ < self.θ_low:
            self.last_trigger = beat_no
            self.hysteresis_count = 0
            return "GROWTH_READY"  # high urgency

        if ΔΦ < self.θ_high:
            self.hysteresis_count += 1
            if self.hysteresis_count >= self.hysteresis:
                self.last_trigger = beat_no
                self.hysteresis_count = 0
                return "GROWTH_EVALUATE"  # standard urgency
        else:
            self.hysteresis_count = 0  # system is still integrating

        return None
```

### 13.2 Lattice Expansion — Rich-Club Topology

```python
def expand_lattice(lattice: Lattice, n_new: int, k_parents: int = 5) -> Lattice:
    """
    Add n_new nodes with rich-club preferential attachment.

    1. Rank existing nodes by total incident edge weight
    2. Select top-k_parents as hub candidates
    3. For each new node:
       a. Sample 3-5 hub nodes (weighted by hub degree)
       b. Initialize edge weights ~Uniform(0.01, 0.05)
       c. Initialize embedding near mean of connected hubs
    4. Return expanded lattice
    """
    # Phase 1: identify hubs
    weights = {
        v: sum(w for _, _, w in lattice.edges(v))
        for v in lattice.nodes
    }
    hubs = sorted(weights, key=weights.get, reverse=True)[:k_parents]

    # Phase 2: add new nodes
    for i in range(n_new):
        v_new = lattice.add_node()
        n_connections = min(len(hubs), random.randint(3, 5))
        targets = random.choices(hubs, weights=[weights[h] for h in hubs], k=n_connections)

        embedding = [0.0] * DIM_EMBEDDING
        for t in targets:
            w = random.uniform(0.01, 0.05)
            lattice.add_edge(v_new, t, weight=w)
            for d in range(DIM_EMBEDDING):
                embedding[d] += lattice.node_embedding[t][d] / n_connections

        lattice.node_embedding[v_new] = [
            e + random.gauss(0, 0.05) for e in embedding
        ]

    return lattice
```

### 13.3 Growth Eligibility — All Six Gates

```python
def check_growth_eligibility(state: SubstrateState) -> GrowthEligibility:
    """Check all six gates. Returns PASS or BLOCKED with reason."""
    checks = []

    # Gate 1: Stage cap
    if state.stage >= MAX_STAGE:
        return GrowthEligibility.BLOCKED("At maximum stage")

    # Gate 2: Stage-specific condition
    if not meets_stage_condition(state):
        return GrowthEligibility.BLOCKED(f"Stage condition not met")

    # Gate 3: Resource check
    if state.disk_free_mb < 500 or state.mem_free_mb < 256:
        return GrowthEligibility.BLOCKED("Insufficient resources")

    # Gate 4: Identity boundary
    if state.boundary not in ("ASSENT", "INTEGRATING"):
        return GrowthEligibility.BLOCKED(f"Boundary: {state.boundary}")

    # Gate 5: Dream debt
    if state.dream_debt >= 3:
        return GrowthEligibility.BLOCKED(f"Dream debt: {state.dream_debt}")

    # Gate 6: Fragmentation stage
    if state.fragmentation_stage >= 2:
        return GrowthEligibility.BLOCKED(
            f"Fragmentation stage: {state.fragmentation_stage}"
        )

    return GrowthEligibility.PASS
```
...
|------|-------------------|
| Detector ignores corrupt beats | Φ=None or NaN → no trigger |
| Detector respects cooldown | Multiple triggers within C beats → only first fires |
| Detector hysteresis prevents flutter | Φ oscillating near threshold → no premature trigger |
| Expansion adds correct N nodes | Lattice node count increases by N_new |
| New nodes connect to high-weight hubs | Degree distribution is skewed toward existing hubs |
| Stage transition fires once | Tools unlock only on first crossing of size threshold |
| Fragmentation stage ≥ 2 blocks growth | Expansion gate returns BLOCKED |
| Dream debt ≥ 3 blocks growth | Expansion gate returns BLOCKED with debt count |
| Rollback restores pre-expansion state | Lattice topology matches pre-expansion snapshot |
| Post-growth monitoring detects failure | Φ not recovered by beat 200 → rollback triggered |
| Journal entry generated on success | Growth completion creates a new journal file |
| Multiple blocked growths enter sleep | 3 consecutive blocks → SLEEP mode with log |

---

*Document updated 2025-07-19 to integrate AXIOMA FragmentationMonitor stages, RecoveryLearner protocol, and Stateful snapshot chain patterns from the existing codebase.*